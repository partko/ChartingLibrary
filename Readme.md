...
> Maximum number of candles: 260000 / (candleWidthInPx + gapWidthInPx)
...
- `topOffset` indentation at the top of the chart in Dp.
...
- `bottomBarHeight` height of the bottom panel of the chart in Dp.
...
### Using AreaChart in your project:
...
> Don't pass more data to the list of AreaFeed than the ConstraintLayout can display (260000 pixels). 
> Maximum areaFeed length: 260000 / priceWidthInPx
...
- `minIndent` the minimum distance between the time tags, measured in the number of candles.
- `dateOffset` offset of the time tags, measured in the number of candles.
- `priceWidth` distance between price points on the chart in Dp.
- `topOffset` indentation at the top of the chart in Dp.
- `rightBarWidth` the thickness of the right panel in Dp.
- `rightBarTextSize` the font size of the prices on the right panel.
- `significantDigits` the number of digits after the decimal point in prices.
- `bottomBarHeight` height of the bottom panel of the chart in Dp.
- `pricePathThickness` the thickness of the price trajectory line in Dp.
- `priceLineThickness` the thickness of the last price line in Dp.
- `priceLineStyle` the effect of the last price line.
- `selectedLineThickness` the thickness of the selected price line in Dp.
- `selectedLineStyle` the effect of the selected price line.
...
if you need to update optional data after creating a chart or if you upload a new list of AreaFeed, you must inform about it as follows, in order to display the changes correctly:
...
fun updateData() {
    ...
    // here we can change the list of AreaFeed and change the optional data
    ...
...
if you need to update the chart in real time by adding one new AreaFeed to the list or changing the last AreaFeed price, you need to inform after each update as follows:
...
fun updateAreaFeed {
    ...
    // here we can add a new AreaFeed to the list, or change the parameters of the last one
    ...
...
- `selectedColor` accent color of the price of the highlighted candle and the corresponding auxiliary line
- `endButtonColor` color of to the end button
- `pricePathColor` color of the price trajectory line
- `gradientStartColor` color of the gradient start at the top of the chart
- `gradientEndColor` color of the gradient end at the bottom of the chart
